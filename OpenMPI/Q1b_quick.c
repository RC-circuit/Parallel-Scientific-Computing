...
int main(int argc, char* argv[])
{
  ...
  MPI_Scatter(output, ln, MPI_DOUBLE, local_output + 2, ln, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  
  for (int j  = 0; j < t/delt; j++){
  if (my_id == 0)
    {
      MPI_Send(local_output + ln, 2, MPI_DOUBLE, my_id + 1, tag, MPI_COMM_WORLD);
    }
  else if (my_id == size -1){
    MPI_Recv(local_output, 2, MPI_DOUBLE, my_id - 1, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }
  else if (my_id % 2 == 0){
    MPI_Send(local_output + ln, 2, MPI_DOUBLE, my_id + 1, tag, MPI_COMM_WORLD);
    MPI_Recv(local_output, 2, MPI_DOUBLE, my_id - 1, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  ...
  if (my_id == 0)
    {
      MPI_Recv(local_output + ln + 2, 1, MPI_DOUBLE, my_id + 1, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
  else if (my_id == size -1){
    MPI_Send(local_output + 2, 1, MPI_DOUBLE, my_id - 1, tag, MPI_COMM_WORLD);
  }
  else if (my_id % 2 == 0){
    MPI_Recv(local_output + ln + 2, 1, MPI_DOUBLE, my_id + 1, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Send(local_output + 2, 1, MPI_DOUBLE, my_id - 1, tag, MPI_COMM_WORLD);
  ...
  for (int i = 1; i < ln+2; i++){
    if ((my_id == 0 && i==2) || (my_id == size-1 && i==ln+1)) {
        continue;
      }
	...
	local_output[i] = upwind_scheme(i, delx, delt, local_output, c);
      ...
      local_output[i] = QUICK_scheme(i, delx, delt, local_output, c);
    }
  }
 }
  ...
  MPI_Gather(local_output + 2, ln, MPI_DOUBLE, output_final, ln, MPI_DOUBLE, 0 , MPI_COMM_WORLD);
